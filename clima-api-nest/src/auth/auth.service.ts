   ...
   */
   async validateSSOToken(token: string, req?: any): Promise<any> {
    ...
    try {
      ...
      // Así mantenemos a Clima sincronizado con los cambios hechos en el Portal Central (nombre cambiado, desactivado, etc)
    ...
    try {
      ...
      if (response.data && response.data.authenticated) {
        ...
        try {
          ...
          console.error('Error on upsert portal session in Clima:', err.message);
        }
      }
    } catch (err) {
      console.error('Error validating portal session in Clima:', err.message);
    }
    return null;
  }
